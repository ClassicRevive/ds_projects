...
for(i in c(1:100000))
{
  ...
  x1=runif(1,12,16)
  y1=runif(1,12,16)
  theta1 = runif(1,pi/2,pi*0.65)  # limits differ for 7's
  len=20
  ...
  params[,i]=c(x1,y1,theta1,width1,x2,y2,theta2,width2,x3,y3,theta3,len3,width3)
...
start <- proc.time()[[3]]
for(i in c(1:1000))nbrs100k[,i]=findnns(b100k,d7[,i])
end <- proc.time()[[3]]
...
start <- proc.time()[[3]]
for(i in c(1:1000))nbrs10k[,i]=findnns(b10k,d7[,i])
end <- proc.time()[[3]]
print(paste0("time taken: ", end-start))
...
dists100k = nearest100k -d7
dists100k = apply(dists100k^2,2,sum)

goodfits=nbrs100k[1,dists100k < 1000000]
